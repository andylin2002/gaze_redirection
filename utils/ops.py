...
def content_loss(endpoints_mixed, content_layers):
    ...
    for layer in content_layers:
        ...
        size = tf.size(feat_a)
        loss += tf.nn.l2_loss(feat_a - feat_b) * 2 / tf.to_float(size) #使用 tf.nn.l2_loss 計算損失，並將其標準化以保持與特徵大小無關

    return loss


def style_loss(endpoints_mixed, style_layers): #使用了 Gram 矩陣來計算兩張圖片在這些層上的風格損失
...
def gram(layer): #Gram 矩陣表示不同特徵之間的相似性
    ...
    grams = tf.matmul(features, features, transpose_a=True) / denominator #歸一化，避免因為feature大小帶來的影響
